...
description: "Use to run DESeq2 differential expression on bulk RNA-seq raw gene-count matrices, including normalization, contrasts, and DE result export."
...
```r
# For analysis-ready plots (required - generates PNG)
...
**License note:** DESeq2 is LGPL (>= 3), and ggprism is GPL (>= 3.0). Commercial use is permitted under these open-source licenses, subject to their license obligations.
...
**QC plots (PNG always, SVG strongly preferred, standard resolution):**
...
- [scripts/qc_plots.R](scripts/qc_plots.R) - Analysis-ready plots with ggplot2/ggprism/ggrepel (PNG + SVG if svglite installed)
...
- `dispersion_plot.svg` - Gene-wise dispersion vs mean expression
...
- Analysis-ready plots styled from the packaged plotting scriptss

**Features built-in:**
- ✅ ggplot2 for customizable, high-quality plots
- ✅ Packaged plotting functions for consistent output
- ✅ ggrepel for non-overlapping text labels
- ✅ Auto-selects vst/rlog by sample size
- ✅ Saves as SVG (vector) or PNG (raster)
...
| **SVG files missing** (only PNG generated) | Install svglite: `install.packages('svglite')`. **Note:** PNG output is identical quality for analysis. | See Installation section |
...
- Analysis-ready visualizations with advanced customization
