...
- `GRCh38.115`: Ensembl release 115, GRCh38
...
```bash
# Download GRCh38
snpEff download GRCh38.115

# Download GRCh37
snpEff download GRCh37.75

# Specify custom data directory
snpEff download -dataDir /path/to/data GRCh38.115
```
...
```bash
snpEff ann GRCh38.115 input.vcf > output.vcf
```
...
- Do not pass `-t N` unless the installed SNPEff version explicitly supports it. SNPEff 5.4c rejects `-t` with "Unknown option".
- For large VCFs, increase Java heap size and split by chromosome instead of relying on a thread flag.
...
```bash
snpEff ann -canon GRCh38.115 input.vcf > output.vcf
```
...
```bash
snpEff ann -hgvs GRCh38.115 input.vcf > output.vcf
```
...
```bash
snpEff ann -lof GRCh38.115 input.vcf > output.vcf
```
...
```bash
snpEff ann -stats report.html GRCh38.115 input.vcf > output.vcf
```
...
snpEff ann -hgvs -lof GRCh38.115 input.vcf | \
...
snpEff ann \
    ...
    -no-downstream \
    -no-upstream \
    GRCh38.115 input.vcf > output.vcf
```
...
for chr in {1..22} X Y; do
    grep "^${chr}\t" input.vcf | snpEff ann GRCh38.115 /dev/stdin > chr${chr}.vcf
done
...
java -Xmx8g -jar snpEff.jar ann GRCh38.115 input.vcf > output.vcf
```
