...
description: "Use to annotate VCF variants with gene and functional consequences, clinical significance, population evidence, and pathogenicity predictions."
...
- Cancer genomics (somatic variant annotation with cancer gene panels)
...
**What you get:**
- **Dataset:** 10 dbSNP-verified BRCA1/BRCA2 variants with GRCh38 REF alleles checked against hg38
- **Expected:** a small mixed SNV/indel VCF suitable for workflow smoke testing; exact consequence counts vary by database version/transcript set
- **Runtime:** VEP ~5 min, SNPEff ~2 min

**Example-data caveat:** Treat the bundled VCF as a workflow smoke test, not a clinical truth set. Do not claim the fixture establishes pathogenicity or clinical actionability; use ClinVar/ACMG-capable annotation resources for real interpretation.
...
# Or: pip install pandas numpy pysam cyvcf2 matplotlib xlsxwriter openpyxl
```
...
```bash
snpEff download GRCh38.115  # Human, ~2-5 GB, ~10 min
```
...
- `analysis_report.pdf` - PDF report generated with the Biomni-native `pdf-report-generation` skill; includes Introduction, Methods, Results, Conclusions, figures, tool/database versions, warnings, and clinical-use limitations
...
1. **Input Files** (ASK THIS FIRST):
   ...
   - **Or use example data for testing?**
     - Use `load_clinvar_pathogenic_sample()` (~10 dbSNP-verified BRCA1/BRCA2 fixture variants)
...
This is a 5-step workflow. Scripts handle annotation/export complexity; the final PDF is assembled with the Biomni-native `pdf-report-generation` skill.
...
indels = stats['insertions'] + stats['deletions']
print(f"Total: {stats['total_variants']}, SNVs: {stats['snvs']}, Indels: {indels}")
```
...
# SNPEff annotation
    ...
    annotated_vcf = run_snpeff(input_vcf, "annotated.snpeff.vcf.gz", genome="GRCh38.115", canon=True, threads=4)
...
**Step 5 - Generate PDF report (MANDATORY):**

The `analysis_report.pdf` output is a contracted deliverable for review and human handoff. This skill package exports data, plots, and `summary_report.xlsx`; it does **not** ship a dedicated PDF-generation script. Use the Biomni-native `pdf-report-generation` skill to assemble the final PDF from the Step 3-4 artifacts.

```python
from biomni.agent import Skill
Skill(action="load", name="pdf-report-generation")
```

The PDF must include:
- **Introduction** — input VCF, organism/build, annotation goal, and selected annotation tool
- **Methods** — VCF validation, VEP or SNPEff configuration, genome/database versions, filters, and plotting/export steps
- **Results** — variant counts, impact/consequence distribution, high/moderate-impact variants, rare variants, and gene summaries
- **Figures** — `consequence_distribution.png`, `impact_by_chromosome.png`, `pathogenicity_scores.png`, `allele_frequency.png`, and `gene_burden.png` when available
- **Warnings and limitations** — reference-build mismatch warnings, missing annotation databases/scores, API/cache fallbacks, and the clinical-use caveat that outputs are not diagnostic without expert review
- **Conclusions and next steps** — prioritized follow-up analyses such as pathway enrichment, protein structure impact mapping, or expert clinical genetics review

**✅ VERIFICATION:** Confirm `results/analysis_report.pdf` exists, has Introduction/Methods/Results/Conclusions sections, embeds readable nonblank figures, and includes any REF mismatch or incomplete-annotation warnings before declaring the workflow complete.

**DO NOT write a custom PDF generator for this skill. Use `pdf-report-generation`.**

---

⚠️ **CRITICAL - DO NOT:**
- ❌ **Write inline validation/annotation/plotting/export code** → **STOP: Use the scripts shown above**
- ❌ **Skip `analysis_report.pdf`** → **STOP: Use `pdf-report-generation` in Step 5**
