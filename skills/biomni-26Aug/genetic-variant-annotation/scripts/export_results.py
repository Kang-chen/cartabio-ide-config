...
def export_to_excel(output_file, sheets, format_columns=True, freeze_panes=(1, 2)):
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        ...
        for sheet_name, df in sheets.items():
            ...
            # Auto-fit columns
            for col_num, column_name in enumerate(df.columns):
                column_widths = df[column_name].fillna('').map(lambda value: len(str(value)))
                max_length = max(
                    column_widths.max(),
                    len(column_name)
                ) + 2
...
def export_to_vcf(df, original_vcf, output_file, add_filter_tag=None, compress=True, index=True):
    ...
    if compress:
        temp_path = output_path.with_suffix('') if str(output_path).endswith('.gz') else output_path
    else:
        temp_path = output_path

    vcf_out = cyvcf2.Writer(str(temp_path), vcf_in)
    ...
    used_bgzip = False
    if compress and temp_path != output_path:
        if shutil.which('bgzip'):
            subprocess.run(['bgzip', '-f', str(temp_path)], check=True)
            compressed_file = Path(f"{temp_path}.gz")

            # Rename to final name
            if compressed_file != output_path:
                compressed_file.rename(output_path)
            used_bgzip = True
        else:
            with open(temp_path, 'rb') as source, gzip.open(output_path, 'wb') as target:
                shutil.copyfileobj(source, target)
            temp_path.unlink()
            print("   Warning: bgzip not found; wrote gzip-compressed VCF without tabix index")

    # Index if requested
    if index and str(output_path).endswith('.gz'):
        if used_bgzip and shutil.which('tabix'):
            subprocess.run(['tabix', '-p', 'vcf', '-f', str(output_path)], check=True)
        elif used_bgzip:
            print("   Warning: tabix not found; skipped VCF index")

    print(f"Exported {n_written} variants to {output_path}")
...
def export_summary_report(
    ...
    """
    af_col = 'gnomAD_AF' if 'gnomAD_AF' in df.columns else 'AF' if 'AF' in df.columns else None
    rare_count = (
        len(df[pd.to_numeric(df[af_col], errors='coerce') < 0.01])
