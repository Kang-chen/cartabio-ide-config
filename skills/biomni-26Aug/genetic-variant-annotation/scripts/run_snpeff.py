...
def download_snpeff_database(genome, data_dir=None):
    ...
    ----------
    genome : str
        Genome identifier (e.g., 'GRCh38.115', 'GRCh37.75', 'GRCm39.105')
...
def run_snpeff(
    ...
    genome : str
        Genome database name (e.g., 'GRCh38.115')
    data_dir : str, optional
    ...
    threads : int
        Kept for API compatibility. SNPEff 5.4c does not accept a stable
        command-line thread flag, so this wrapper does not pass one.
    ...
    if threads > 1:
        print("Note: SNPEff thread flag is not passed; SNPEff 5.4c rejects '-t'.")
    ...
    try:
        env = os.environ.copy()
        env.setdefault("JAVA_TOOL_OPTIONS", f"-Xmx{DEFAULT_SNPEFF_JAVA_HEAP}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--genome', help='Genome database (e.g., GRCh38.115)')
