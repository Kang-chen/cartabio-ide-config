...
|---------|---------|---------|----------------|---------|
| **RNASeqPower** | ≥1.36.0 | LGPL (>=2) | ✅ Permitted | RNA-seq power calculations |
...
| **sva** | ≥3.48.0 | Artistic-2.0 | Surrogate variable analysis examples in `batch_effect_mitigation.md`; not used by packaged workflow scripts |
| **osat** | ≥0.1.3 | Artistic-2.0 | Legacy optimal-assignment examples; not used by packaged workflow scripts |
...
BiocManager::install(c("PROPER", "SingleCellExperiment", "sva"))
install.packages(c("ashr", "locfdr", "blockTools", "osat"))
```
...
packages_to_test <- c("RNASeqPower", "ssizeRNA", "DESeq2", "qvalue", "IHW", "ggplot2", "ggprism")
all_installed <- TRUE
...
The license columns in the package tables reflect each package's stated license at the time of packaging. Before relying on any dependency for commercial use, verify the current license terms directly (e.g., via `packageDescription(<pkg>)$License` in R or the CRAN/Bioconductor page). GPL-licensed packages (RnaSeqSampleSize, pwr, ggprism) permit execution but may trigger copyleft obligations if redistributed or incorporated into a distributed product — consult legal counsel for commercial deployment scenarios. The `pasilla` dataset (Artistic-2.0) is used here as built-in example pilot data only; commercial use of the dataset itself should be verified against its original source (Brooks et al. 2011).
