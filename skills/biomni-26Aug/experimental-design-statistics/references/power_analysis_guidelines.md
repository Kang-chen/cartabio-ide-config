...
### Variance Decomposition: CV, Dispersion, and lambda0

RNA-seq count data follows a negative-binomial model whose total variance decomposes as:

**CV_total^2 = 1/lambda0 + BCV^2**

where:
- **lambda0** is the per-gene expected count (the number of reads expected for a single gene at the given library size). This is NOT the library size in reads or millions of reads.
- **BCV** (biological coefficient of variation) is the square root of the dispersion parameter phi. It captures between-replicate biological variability.
- **CV_total** is the total coefficient of variation, combining the Poisson sampling term (1/lambda0) and the biological term (BCV).

**Critical distinction:** Library depth in reads is NOT a direct input to RNASeqPower or RnaSeqSampleSize. The model input is lambda0 (per-gene expected count), which depends on library size but is a per-gene quantity typically in the range 5-300. Increasing library size increases lambda0, which shrinks the Poisson term 1/lambda0 — but at typical BCV values (0.1-0.4) the biological term dominates and the effect on power is small relative to changing n or BCV. The single source of truth for this identity is `scripts/variance_model.R`.
...
power <- calc_power_rnaseq(
  mean_count = 20,     # mean counts per gene (lambda0)
