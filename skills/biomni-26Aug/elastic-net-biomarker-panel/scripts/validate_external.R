# External Cohort Validation for Elastic-Net Biomarker Panel
...
#'
#' Applies the locked biomarker panel model to external validation data and computes
...
#' @param accession GEO accession of THIS cohort (e.g. "GSE32646"), if known
#' @param discovery_accession GEO accession of the discovery cohort, for the
#'   same-study guard (a matching accession means this is a held-out split)
#' @param discovery_sample_ids Optional discovery sample IDs; any overlap with
#'   this cohort's sample IDs also flags a same-study held-out split
#'
#' @return list(auc, auc_ci, roc_object, predictions, performance_table,
#'   cohort_name, accession, is_same_study, cohort_type, ...)
validate_panel <- function(model_result, validation_X, validation_y,
                            cohort_name = "validation",
                            accession = NULL, discovery_accession = NULL,
                            discovery_sample_ids = NULL) {

    # A cohort drawn from the same accession as discovery, or sharing sample IDs
    # with it, is a HELD-OUT SPLIT (same study/platform) — NOT external validation.
    is_same_study <- (!is.null(accession) && !is.null(discovery_accession) &&
                      identical(as.character(accession),
                                as.character(discovery_accession))) ||
                     (!is.null(discovery_sample_ids) &&
                      length(intersect(rownames(validation_X), discovery_sample_ids)) > 0)
    cohort_type <- if (isTRUE(is_same_study)) {
        "held-out split (same study/platform)"
    } else "external"
    label <- if (isTRUE(is_same_study)) {
        "Held-out Split (Same Study/Platform)"
    } else "External Validation"

    cat("\n===", label, ":", cohort_name, "===\n\n")
    if (isTRUE(is_same_study)) {
        warning("validate_panel(): validation cohort shares the discovery ",
                "accession/samples; this is a held-out split (same study/platform), ",
    ...
    cat("  NOTE: threshold, sensitivity, specificity, PPV and NPV were chosen at the\n")
    cat("        Youden-optimal cut-point on THIS set (in-sample); treat as optimistic.\n")
    ...
    return(list(
        ...
        accession = accession,
        is_same_study = isTRUE(is_same_study),
