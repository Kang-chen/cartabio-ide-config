# Load Example Data for Elastic-Net Biomarker Panel Skill
...
load_unifi_data <- function(data_dir = "data", endpoint = "mucosal_healing") {
    ...
    return(list(
        ...
        report_context = list(
            ...
            # report_context: DATASET + CLINICAL QUESTION only. Never hardcode analysis
            # parameters (n_repeats, train_fraction, stability_threshold, alpha) here —
            # they are printed in Methods from model_result$parameters. Hardcoding them
            # will trip .assert_report_consistency().
            analytical_goals = c(
    ...
    list(
        ...
        analytical_goals = c(
            ...
            paste("Evaluate panel performance through rigorous repeated nested",
                  "cross-validation with stability selection to ensure robust",
                  "classification and unbiased AUC estimation."),
        ...
        # Spec item 14: tag each goal with its requirement level so the report can
        # flag goals that are out of scope or require downstream interpretation.
        # 'core' = addressed by this skill's pipeline; 'out_of_scope' = not
        # addressable by cross-sectional penalised regression (needs survival analysis);
        # 'interpretation' = requires downstream biological interpretation module.
        analytical_goal_requires = c("core", "core", "out_of_scope", "interpretation"),
        published_benchmarks = list(
            intro = paste(
    ...
    list(
        ...
        analytical_goals = c(
            ...
            paste("Evaluate signature stability through repeated nested cross-validation",
                  "with stability selection to ensure robust feature selection and",
                  "unbiased AUC estimation in this moderately-sized cohort."),
            paste("Characterize selected biomarkers through immune pathway enrichment,",
                  "blood cell-type expression mapping via CZI CELLxGENE Census, and",
