# Prepare Feature Matrix for Elastic-Net Biomarker Panel Selection
# Transforms expression data into a model-ready feature matrix

#' Prepare feature matrix for penalised regression modeling
#'
...
#' @param top_n_variable Number of most variable features to retain (default:
#'   NULL = no variance filtering here). Variance filtering and scaling belong
#'   inside run_lasso_panel()'s folds (preprocess_per_fold=TRUE) for a
#'   leakage-free analysis. A non-NULL value here fixes the candidate pool on
#'   the whole cohort upstream, which downgrades leakage_status to PARTIAL.
#' @param log_transform Apply log2(x+1) transformation (default: TRUE for TPM data)
#' @param scale_features Scale features to zero mean/unit variance (default:
#'   FALSE). Scaling belongs inside run_lasso_panel()'s folds for a
#'   leakage-free analysis. TRUE here is a legacy path that downgrades
#'   leakage_status.
#'
#' @return list(X, y, feature_names, sample_ids, transform_params, prep_params)
prepare_feature_matrix <- function(expression, metadata, outcome_col,
                                    top_n_variable = NULL,
                                    log_transform = TRUE,
                                    scale_features = FALSE) {
    cat("Preparing feature matrix for penalised regression...\n")
    ...
    # Record input feature count before any variance filtering (for prep_params)
    n_input_features <- nrow(expr)

    # Filter to top N most variable features (NULL = no filter)
    global_variance_filter_n <- NA
    if (!is.null(top_n_variable) && top_n_variable < nrow(expr)) {
        feature_vars <- apply(expr, 1, var)
        top_idx <- order(feature_vars, decreasing = TRUE)[1:top_n_variable]
        expr <- expr[top_idx, , drop = FALSE]
        global_variance_filter_n <- top_n_variable
    ...
    if (scale_features) {
        ...
        # Transpose: glmnet expects samples x features
        X <- t(expr_scaled)
    } else {
        X <- t(expr)
    }
    ...
    # Record preprocessing provenance so run_lasso_panel() can derive
    # leakage_status. The attribute survives positional passing of X.
    prep_params <- list(
        global_variance_filter_n = global_variance_filter_n,
        global_scaling = scale_features,
        log_transformed = log_transform && max(expr, na.rm = TRUE) > 100,
        n_input_features = n_input_features
    )
    attr(X, 'prep_params') <- prep_params
