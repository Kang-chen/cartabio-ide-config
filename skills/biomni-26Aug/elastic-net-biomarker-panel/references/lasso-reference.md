# Penalised Regression Parameter Reference Guide
...
**Recommendation:** This skill's workflow defaults to `alpha = 0.5` (elastic net), which retains correlated features. If you need the sparsest possible panel (e.g., clinical assay development), use `alpha = 1.0` (pure LASSO). If no features pass stability selection at `alpha = 1.0`, try `alpha = 0.5`.
...
### Selection Frequency
The fraction of CV iterations where a feature had a non-zero coefficient.
...
## Advanced: WGCNA → Penalised Regression Cascade
...
2. Use hub genes (top connectivity per module) as input features for `run_lasso_panel()`
3. This reduces thousands of genes to hundreds of biologically coherent candidates
4. The penalised model then selects the minimal panel from these candidates
