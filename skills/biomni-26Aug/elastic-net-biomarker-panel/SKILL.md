...
name: "elastic-net-biomarker-panel"
description: "Use to select compact predictive biomarker panels with elastic-net or pure-LASSO logistic regression, nested cross-validation, stability selection, and independent-cohort validation."
category: "multi_omics"
visibility: "public"
starting-prompt: "Build a 5-15 gene biomarker panel from the MARS sepsis consortium whole-blood transcriptomics dataset (GSE65682, Affymetrix HG-U219) that classifies immunosuppressed Mars1 ICU sepsis patients against all other endotypes, using elastic-net logistic regression with nested cross-validation and stability selection, then validate the locked panel on an independent cohort. Generate a PDF report with intro, methods, results, conclusions, and figures from all analyses performed."
---

# Elastic-Net Biomarker Panel Discovery & Validation

Select minimal, interpretable biomarker panels from high-dimensional omics data using penalised logistic regression (elastic net by default, pure LASSO as an option) with nested cross-validation and stability selection.

> **Historical naming note:** The script file `scripts/lasso_workflow.R` and the R function `run_lasso_panel()` retain their original names for backward compatibility. Despite the name, `run_lasso_panel()` fits the full elastic-net family — set `alpha = 1.0` for pure LASSO, `alpha = 0.5` for elastic net (the recommended default in this skill's workflow), or any value in between.
...
- **Neurodegeneration progression / trajectory prediction** (Alzheimer's, Parkinson's, ALS, FTD) — cross-sectional biomarker panels cannot model disease progression. Use Cox proportional hazards, mixed-effects models, or estimated-years-of-onset (EYO) anchored longitudinal designs instead. See Scientific Caveats.
...
**Primary results:**
- `biomarker_panel.csv` — Final panel features with model coefficients and selection frequencies
- `all_feature_stability.csv` — All features ranked by selection frequency
- `discovery_performance.csv` — Per-fold AUC, sensitivity, specificity (full elastic-net model only)
- `locked_panel_performance.csv` — Per-fold AUC for the locked stable-feature panel (selection-biased; see Scientific Caveats)
- `locked_panel_cv_predictions.csv` — Held-out predictions from locked-panel CV
- `cv_predictions.csv` — Held-out predictions from the main CV
- `validation_performance.csv` — External validation metrics (if validation cohort provided)
- `parameters.csv` — All analysis parameters (alpha, n_repeats, train_fraction, stability_threshold, preprocessing settings, leakage_status)
- `report_consistency_check.csv` — Automated consistency gate results (verifies report text matches exported files)
...
- `final_glmnet_model.rds` — The final fitted cv.glmnet object on stable features (for coefficient extraction or re-prediction)
...
- `coefficient_forest.png/.svg` — Model coefficients with 95% CIs
...
- `summary_report.md` — machine-generated markdown content source written by `export_all()` (sections, tables, caveats, citations, and the automated consistency gate); it is the single content source the report step renders from, not a standalone report surface.
...
### 1. **Example or Own Data?** (ASK THIS FIRST):
   - **a) Run example dataset to showcase the workflow** (recommended)
     - Runs a sepsis blood transcriptomics demo (MARS Consortium, ICU patients) that derives a ~15–25 gene panel for identifying immunosuppressed sepsis patients — both the full-model CV AUC and the locked-panel CV AUC are reported in the exported files (panel size and AUCs vary across runs)
...
### 3. **Outcome & Organism** *(own data only)*:
   ...
   - **Trajectory check:** Is the outcome a cross-sectional binary label derived from a progressive neurodegenerative disease (Alzheimer's, Parkinson's, ALS, FTD)? If so, this skill is not appropriate — disease progression requires longitudinal modeling (Cox proportional hazards, mixed-effects models, or EYO-anchored designs). Redirect the user to `survival-analysis-clinical` or `disease-progression-longitudinal`.
...
**Step 1 - Load data:**
*Rationale: Standardized loaders ensure consistent sample/outcome alignment and attach the `report_context` (dataset description + clinical question) that the report uses. Custom loading risks mismatched sample IDs or non-binary outcomes.*
...
**Step 2 - Run biomarker panel analysis:**
*Rationale: Penalized logistic regression with stability selection produces sparse, interpretable panels from high-dimensional omics data. Nested cross-validation provides an honest within-cohort AUC estimate. Preprocessing (variance filtering, scaling) is now performed inside CV folds to prevent train/test leakage.*

```r
source("scripts/prepare_features.R")
features <- prepare_feature_matrix(data$expression, data$metadata, data$outcome_col)

source("scripts/lasso_workflow.R")
model <- run_lasso_panel(features$X, features$y, alpha = 0.5, top_n_variable = 2000)
```
**DO NOT write inline LASSO code (glmnet, cv.glmnet, etc.). Just use the scripts.**

**Note:** `prepare_feature_matrix()` no longer applies variance filtering or scaling by default — those are now handled inside `run_lasso_panel()`'s cross-validation folds (set `top_n_variable` and `preprocess_per_fold` there) to prevent train/test leakage.
...
*Rationale: ROC curves, stability barplots, and calibration plots communicate discriminative performance, feature robustness, and prediction reliability — the three pillars of biomarker panel evaluation.*
...
**Step 4 - Export results (MANDATORY — do not skip):**
*Rationale: The exported report is the deliverable. The consistency gate verifies that every AUC and parameter in the report text is derived from exported files — not hardcoded — making the recurring restatement defect structurally impossible.*
...
**This step is mandatory.** `export_all()` writes all CSV exports, the analysis figures, the RDS model objects, the `report_consistency_check.csv` gate, and `summary_report.md` — the single machine-generated content source that the report step (Step 6) renders from. The consistency gate verifies that every AUC and parameter stated in the report text is derived from the exported files, not hardcoded.
...
val_result <- validate_panel(model, val_features$X, val_data$metadata[[val_data$outcome_col]],
                              cohort_name = "GSE32646", accession = val_data$accession,
                              discovery_accession = data$accession)
export_all(model, validation_result = val_result, output_dir = "results",
           data = data, features = features)
```

**Step 6 — Generate the PDF report (MANDATORY terminal step — the run is not complete until this step has happened):**
*Rationale: The shareable report is the actual deliverable a downstream reader (PI, collaborator, regulator) sees. Routing it through the platform reporting skill guarantees consistent styling and a single source of truth, so the report cannot contradict the exported numbers.*
...
4. **Never use the word "validated"** to describe a panel that has only been tested in the discovery cohort
5. **Always cite `leakage_status` verbatim** from `parameters.csv` — do not paraphrase or hardcode a leakage claim. The report's consistency gate checks that the leakage status in the text matches the exported value.
6. **Always cite the locked-panel CV AUC** from `locked_panel_performance.csv` alongside the full-model CV AUC, and note that the locked-panel AUC is selection-biased (the panel was chosen on the same data). Do not present the locked-panel AUC as an independent validation.
...
| Figure | Source file | What it means | Use this? |
|--------|-------------|---------------|-----------|
| Mean CV AUC | `discovery_performance.csv` | Average AUC across held-out test sets | **YES — cite this** |
| Final model AUC | ROC curve plot annotation | Model applied back to its own training data | **NO — in-sample, optimistic** |
| Locked-panel CV AUC | `locked_panel_performance.csv` | AUC of the stable-feature panel (selection-biased) | **Report alongside, but flag as selection-biased** |
...
**Expected magnitude of bias:** Typically 0.02–0.05 AUC units for datasets of this size. A high discovery AUC should be interpreted as "likely lower in an independent cohort if the signal is real" — not as a guaranteed performance floor.

**The only way to get an unbiased estimate is external validation.**

## Scientific Caveats

1. **Discovery AUC is optimistic.** The mean CV AUC from `discovery_performance.csv` is an in-cohort estimate subject to optimism bias (feature stability is computed on the same samples). Treat it as an upper bound, not a validated performance figure. Only external validation (Step 5) provides an unbiased estimate.

2. **Locked-panel CV AUC is selection-biased.** The `locked_panel_performance.csv` AUC re-scores the stable-feature panel in the same fold structure used to select it. Because the panel was chosen on the same data, this AUC is upward-biased relative to true out-of-sample performance. It is reported for transparency (showing the gap between full-model and locked-panel performance), not as a validation. Never present it as an independent estimate.

3. **Cross-sectional design cannot model progression.** This skill classifies a binary outcome at a single time point. It is not appropriate for neurodegenerative disease progression (Alzheimer's, Parkinson's, ALS, FTD) or any trajectory prediction. Those require longitudinal models — Cox proportional hazards, mixed-effects regression, or estimated-years-of-onset (EYO) anchored designs. See `survival-analysis-clinical` or `disease-progression-longitudinal`.

4. **Panel biology requires downstream enrichment.** Gene names alone do not establish biological mechanism. Do not interpret panel gene functions without running `functional-enrichment-from-degs` or equivalent pathway analysis in this session. Stating gene functions from memory is a hallucination risk.
...
| Skill | Relationship |
|-------|-------------|
| `bulk-rnaseq-counts-to-de-deseq2` | **Upstream** — DE results can pre-filter features for the penalised model |
| `coexpression-network` | **Upstream** — WGCNA hub genes as penalised model candidates (Paper 1 cascade) |
