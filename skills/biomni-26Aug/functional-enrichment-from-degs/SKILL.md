...
description: "Use to run clusterProfiler functional enrichment on differential-expression results, including over-representation analysis and GSEA with exportable results and plots."
...
**⚠️ KEGG requires commercial license for commercial use.** MSigDB and GO use CC-BY 4.0 (commercial use permitted with attribution). For commercial applications, use Hallmark, Reactome, or GO databases by default. Use KEGG only when the user explicitly asks for it or confirms the required commercial-use license.
...
- Method: GSEA
- Databases: Hallmark + Reactome by default for commercial-safe pathway coverage; KEGG is opt-in only after license confirmation
- Species: Human
...
3. **Databases:** Hallmark + Reactome (commercial-friendly default), GO:BP (detailed processes), KEGG (opt-in; requires commercial license confirmation), C6/C7 (cancer/immunology)
...
# Prepare inputs
ranked_genes <- create_ranked_list(de_results)
term2gene <- get_msigdb_genesets("human", c("H", "REACTOME"))
```
...
- [scripts/generate_plots.R](scripts/generate_plots.R) - Creates analysis-ready PNG + SVG plots **with automatic SVG fallback handling**
...
2. **Database selection (Step 3):** Hallmark + Reactome (commercial-friendly default), GO:BP (detailed processes), KEGG (opt-in; requires commercial license confirmation), or C6/C7 (cancer/immunology)
...
Use CSV results for supplementary tables, SVG plots for figures, and include enrichment summary in results section. Document database versions, parameters, and software versions in methods.
