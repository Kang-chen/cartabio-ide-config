...
generate_ora_barplot <- function(ora_result, direction, top_n = 15, output_file = NULL) {
  ...
  if (is.null(output_file)) {
    direction_key <- tolower(direction)
    direction_key <- gsub("[^a-z0-9]+", "_", direction_key)
    direction_key <- gsub("^_+|_+$", "", direction_key)
    direction_key <- sub("^upregulated$", "up", direction_key)
    direction_key <- sub("^downregulated$", "down", direction_key)
    output_file <- sprintf("ora_%s_barplot.svg", direction_key)
  }
