...
#'
#' # Hallmark + Reactome (commercial-friendly default)
#' term2gene <- get_msigdb_genesets(species = "human", categories = c("H", "REACTOME"))
#'
#' # KEGG legacy pathways (requires commercial-use license review)
...
get_msigdb_genesets <- function(species = "human", categories = c("H")) {
  ...
  cat_map <- list(
    ...
    "KEGG" = list(collection = "C2", subcollection = "CP:KEGG_LEGACY"),  # KEGG legacy (msigdbr >=10)
    ...
    "C2:CP:KEGG" = list(collection = "C2", subcollection = "CP:KEGG_LEGACY"),
    "C2:CP:KEGG_LEGACY" = list(collection = "C2", subcollection = "CP:KEGG_LEGACY"),
