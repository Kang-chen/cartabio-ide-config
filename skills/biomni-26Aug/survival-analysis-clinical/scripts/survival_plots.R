...
# Null-coalescing operator (defined locally so this script is self-contained)
...
plot_km_overall <- function(result) {
    ...
    endpoint_label <- result$endpoint_label %||% "Overall Survival"
    ...
    if (.has_survminer) {
        p <- ggsurvplot(
            ...
            surv.median.line = "hv",
            palette = "#2E86C1",
            title = paste(endpoint_label, "-", result$dataset_name),
            xlab = "Time (years)",
    ...
    ggplot(surv_data, aes(x = time, y = surv)) +
        ...
        labs(title = paste(endpoint_label, "-", result$dataset_name),
             x = "Time (years)", y = paste(endpoint_label, "Probability")) +
...
plot_forest <- function(result) {
    coef_df <- result$cox$coefficients
    df <- coef_df
    ...
    # Build subtitle: significance stars + PH violation warning if applicable
    subtitle <- "* p<0.05, ** p<0.01, *** p<0.001"
    if (!is.null(result$ph_test)) {
        global_p <- result$ph_test$table["GLOBAL", "p"]
        if (!is.na(global_p) && global_p < 0.05) {
            subtitle <- paste0(
                "PH assumption violated (global p = ",
                format.pval(global_p, digits = 3),
                ") — HRs are time-averaged and may be misleading.\n",
                subtitle)
        }
    }
...
plot_risk_groups <- function(result) {
    ...
    if (.has_survminer) {
        p <- ggsurvplot(
            ...
            conf.int = TRUE,
            palette = colors,
            title = paste(endpoint_label, "by Risk Group -", result$dataset_name),
    ...
    ggplot(surv_df, aes(x = time, y = surv, color = strata)) +
        ...
        scale_color_manual(values = colors) +
        scale_y_continuous(labels = scales::percent_format(), limits = c(0, 1)) +
        labs(title = paste(endpoint_label, "by Risk Group -", result$dataset_name),
             subtitle = paste("Log-rank p =", format.pval(lr_p, digits = 3)),
             x = "Time (years)", y = paste(endpoint_label, "Probability"),
             color = "Risk Group") +
        theme_prism(base_size = 12) +
...
plot_cumulative_hazard <- function(result) {
    ...
    ggplot(cumhaz_df, aes(x = time, y = cumhaz)) +
        geom_step(color = "#2E86C1", linewidth = 1) +
        labs(title = paste(endpoint_label, "- Cumulative Hazard -",
                           result$dataset_name),
