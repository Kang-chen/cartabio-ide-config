...
For categorical covariates, `fit_cox_model()` automatically selects a clinically meaningful reference level using a four-rule precedence, applied in order until one succeeds:

1. **Ordered factor → lowest level.** If the column is an ordered factor (e.g., `size` with levels `<=20 < 20-50 < >50`), the lowest level is the reference.
2. **Clinical convention.** A lookup table keyed on column name and level values maps known clinical variables to their conventional reference: `grade`/`histologic_grade` → lowest grade present in the data (never hardcoded to a level that may be absent); `stage` → lowest present; `nodes`/`node_status`/`lymph_node` → `Negative`/`N0`/`0`; `er_status`/`pr_status`/`her2_status` → `Negative`; `ecog_ps`/`performance_status` → lowest; `hormon`/`chemo`/`therapy`/`treatment` → `No`/`None`/`0`/`Untreated`; `age_group` → lowest band; `risk_group` → `Low Risk`; `meno` → `Premenopausal`.
3. **Ordinal-lowest.** If level strings contain parseable numbers (e.g., `Stage I`, `Grade 2`), the level with the smallest leading number is chosen. `<` prefixes are treated as below the stated value; `>` prefixes as above.
4. **Largest group (fallback).** If none of the above rules apply, the largest group is used and the rule is recorded as `largest-group`.

The selected reference level, its N, and the rule that selected it are recorded in `result$reference_levels[[covariate]]` as a list with elements `reference`, `n`, and `rule` (one of `ordered-factor`, `clinical-convention`, `ordinal-lowest`, `largest-group`). These are also exported to `results/reference_levels.csv`.

**Small reference groups:** If the selected reference has N < 50, a warning is emitted. Small reference groups produce wide CIs and unstable HRs — always check N in the reference group.

**Override:** To manually specify reference levels, pass `reference_levels` to `fit_cox_model()` or `run_survival_analysis()` as a named list, e.g., `reference_levels = list(grade = "Grade 3")`. User-specified overrides take precedence over all automated rules.
