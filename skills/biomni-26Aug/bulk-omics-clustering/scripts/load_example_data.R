...
#' dataset from Bioconductor. It is a demo/validation fixture with 128 ALL
#' samples and B-cell/T-cell subtype labels. The Bioconductor phenotype data is
#' mixed-age, so do not describe this dataset as pediatric-only.
#'
...
#' - Contains 128 ALL samples from the Bioconductor ALL package
#' - Main labels: B-cell ALL (95 samples) vs T-cell ALL (33 samples)
...
load_example_clustering_data <- function(n_top_variable_genes = 1000) {
  ...
  cat("  Source: Bioconductor ALL demo dataset - 128 ALL samples with B/T labels\n")
  ...
  list(
    ...
    description = sprintf(
      "Bioconductor ALL (Acute Lymphoblastic Leukemia) demo microarray data: 128 mixed-age ALL samples with B-cell or T-cell subtype labels. Use only as a workflow validation/example dataset when no user dataset is provided. Data is log2-transformed, quantile-normalized, filtered to %d most variable genes. Citation: Chiaretti et al. (2004) Blood 103(7):2771-2781",
